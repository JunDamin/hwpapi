"""
Dataclass wrappers around HWP ParameterSets for character, paragraph, and page shapes.

These Python dataclasses provide Pythonic attribute access to HWP's
equivalent ``ParameterSet`` data, useful for snapshotting, comparing,
and manipulating document state without raw COM calls.

- :class:`Character`: Character-level attributes (Bold, FaceName*, etc.) — dataclass
- :class:`CharShape`: Wrapper with ``.p`` holding a ``Character`` dataclass,
  provides methods for reading/writing via HWP actions
- :class:`Paragraph`: Paragraph-level attributes — dataclass
- :class:`ParaShape`: Wrapper with ``.p`` holding a ``Paragraph`` dataclass
- :class:`PageShape`: Minimal page attribute dataclass

NOTE: These are distinct from the ``CharShape`` / ``ParaShape`` in
:mod:`hwpapi.parametersets.sets.primitives` — those are the full 65/32
property ParameterSet classes, these are simpler dataclass wrappers.
"""
