...
def get_text(app):
    """Return current document text as string."""
    txt = app.get_text()
...
def scenario_01_sequential_find(app):
    ...
    with Scenario("1. Sequential find — 반복 검색") as s:
        clear_doc(app)
        app.insert_text("Apple과 Apple. Apple 세 번째 Apple. 마지막 Apple입니다.\n")

        app.move.top_of_file()

        # find_text jumps to FIRST occurrence
        first = app.find_text("Apple")
        s.log(f"first find_text: {first}")

        # Keep running RepeatFind via action
        matches = 0
        for i in range(6):
            try:
                # Using RepeatFind action
        ...
        s.log(f"total iterations: {matches}")
        snap(app, "01_sequential_find")


def scenario_02_charshape_combinations(app):
    """Apply many CharShape attribute combinations."""
    with Scenario("2. CharShape 조합 — bold/italic/underline/color/size") as s:
        clear_doc(app)
        ...
        combos = [
            ...
            ("빨강\n", {"text_color": "#FF0000"}),
            ("큰 글자\n", {"height": 2400}),  # 24pt
            ("작은 굵은 파랑\n", {"bold": True, "height": 800, "text_color": "#0000FF"}),
            ("모두 적용\n", {"bold": True, "italic": True, "underline_type": 1,
                             "text_color": "#00AA00", "height": 1400}),
        ]

        for text, opts in combos:
            # Insert text first, then select and format
            before_len = len(get_text(app))
            app.insert_text(text)
            # Move back to start of newly inserted text
            for _ in range(len(text)):
                app.api.Run("MoveSelLeft")
            if opts:
                app.set_charshape(**opts)
            app.move.bottom_of_file()
            s.log(f"{text.strip()!r:30} → {opts}")

        snap(app, "02_charshape_combos")


def scenario_03_parashape(app):
    """Paragraph shape: alignment, spacing."""
    with Scenario("3. ParaShape — 정렬/여백/줄간격") as s:
        clear_doc(app)

        # Left-align
        app.insert_text("왼쪽 정렬 문단입니다. " * 5 + "\n")
        app.move.top_of_file()
        app.select_text()
        app.set_parashape(align_type=1)  # left
        s.log("left alignment")
        ...
        s.log("center alignment")

        # Right
        app.move.bottom_of_file()
        app.insert_text("오른쪽 정렬 문단입니다.\n")
        # select last line
        app.api.Run("MoveLineUp")
        app.select_text()
        app.set_parashape(align_type=2)  # right
        s.log("right alignment")
...
def scenario_05_find_with_format(app):
    ...
    with Scenario("5. Find with format filter — 굵은 글자만 찾기") as s:
        ...
        s.log(f"FindCharShape.Bold = {ps.find_char_shape.Bold}")
...
def scenario_07_page_setup(app):
    """Page margins with unit strings."""
    with Scenario("7. Page setup — 단위 문자열 ('210mm' 등)") as s:
        action = app.actions.PageSetup
        ps = action.pset

        # Use UnitProperty's mm strings
        try:
            ps.top_margin = "20mm"
            ps.bottom_margin = "20mm"
            ps.left_margin = "25mm"
            ps.right_margin = "25mm"
            s.log(f"set margins: top={ps.top_margin}, bottom={ps.bottom_margin}, "
                  f"left={ps.left_margin}, right={ps.right_margin}")
        except Exception as e:
            s.log(f"unit string failed: {e}, falling back to integers")
            ps.top_margin = 5661   # 20mm in HWPUNIT
            ps.bottom_margin = 5661
            ps.left_margin = 7076  # 25mm
            ps.right_margin = 7076

        # Don't run — just verify property access works
        s.log("(not executing PageSetup.run to preserve existing layout)")
...
def scenario_08_clone_merge(app):
    ...
    with Scenario("8. Native pset methods — clone/merge/is_equivalent") as s:
        ...
        s.log(f"after update_from target.Bold=1: ps.Bold={ps.Bold}")
...
def scenario_09_array_property(app):
    ...
    with Scenario("9. Array properties — TabDef tab_stops") as s:
        try:
            ...
            # Find an array-like property
            from hwpapi.parametersets import ArrayProperty
            for name, desc in type(ps)._property_registry.items():
                if isinstance(desc, ArrayProperty):
                    s.log(f"  array prop: {name}")
        except Exception as e:
            s.log(f"ParaShape inspect: {e}")

        # TabDef directly
        try:
            from hwpapi.parametersets import TabDef
            td = TabDef()
            s.log(f"TabDef created, attributes: {td.attributes_names}")
        except Exception as e:
            s.log(f"TabDef error: {e}")


def scenario_11_bulk_insert(app):
    """Bulk text insertion — stress test."""
    with Scenario("11. Bulk insert — 100 lines") as s:
        clear_doc(app)
        start = time.perf_counter()
        for i in range(100):
            app.insert_text(f"Line {i + 1}: 테스트 반복 텍스트입니다.\n")
        elapsed = (time.perf_counter() - start) * 1000
        s.log(f"inserted 100 lines in {elapsed:.0f} ms ({elapsed / 100:.1f} ms/line)")

        text = get_text(app)
        lines = text.count("\n")
        s.log(f"total lines in doc: {lines}")
...
def scenario_12_introspection_chain(app):
    ...
    with Scenario("12. Introspection chain — no COM") as s:
        # List all actions with specific pset types
...
def scenario_13_unit_conversion(app):
    ...
    with Scenario("13. Unit conversion — mm/cm/in/pt") as s:
        ...
        cases = [
            ("210mm", 210 * 283),
            ("21cm", 21 * 2830),
            ("8.27in", int(8.27 * 7200)),
            ("12pt", 12 * 100),
        ]
        for unit_str, expected in cases:
            result = to_hwpunit(unit_str)
            ok = abs(result - expected) < 5
            s.log(f"{unit_str:10s} → {result:7d} (expected ~{expected}) {'✓' if ok else '✗'}")

        # Round-trip
        for mm in [10, 100, 210]:
            hwp = to_hwpunit(f"{mm}mm")
            back = from_hwpunit(hwp, "mm")
            s.log(f"{mm}mm → {hwp} HWPUNIT → {back}mm")
...
def scenario_10_save_reopen(app, tmpdir):
    ...
    with Scenario("10. Full save/reopen cycle") as s:
        ...
        if marker in text_after:
            s.log("marker preserved after reopen")
        else:
            raise AssertionError(f"marker '{marker}' not found after reopen: {text_after!r}")
